    ...
    private ArcConfiguration readFromAttributes(AttributeSet attributeSet) {
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            if (type == R.styleable.SimpleArcLoader_arc_thickness) {
                float value = array.getDimension(R.styleable.SimpleArcLoader_arc_thickness, getContext().getResources().getDimension(com.intuit.sdp.R.dimen._3sdp));
    ...
    private static class ArcDrawable extends Drawable implements Animatable {
        ...
        public void draw(Canvas canvas) {
            ...
            if (mDrawCirle) {
                ...
                canvas.drawCircle((float) w / 2, (float) h / 2, (float) w / 2, mPaint);
